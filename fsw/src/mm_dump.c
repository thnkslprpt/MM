    ...
    */
    ...
    else
    {
        CFE_EVS_SendEvent(MM_PSP_READ_ERR_EID, CFE_EVS_EventType_ERROR,
                          "PSP read memory error: RC=%d, Address=%p, MemType=MEM%u", (int)PSP_Status,
...
bool MM_DumpMemToFileCmd(const CFE_SB_Buffer_t *BufPtr)
{
    ...
    if (Valid == true)
    {
        /* Run necessary checks on command parameters */
        Valid =
            MM_VerifyLoadDumpParams(SrcAddress, CmdPtr->Payload.MemType, CmdPtr->Payload.NumOfBytes, MM_VERIFY_DUMP);
            ...
            */
            ...
            if (OS_Status == OS_SUCCESS)
            {
                ...
                if (Valid == true)
                {
                    ...
                    if (Valid == true)
                    {
                        CFE_EVS_SendEvent(MM_DMP_MEM_FILE_INF_EID, CFE_EVS_EventType_INFORMATION,
                                          "Dump Memory To File Command: Dumped %d bytes from address %p to file '%s'",
                                          (int)MM_AppData.HkPacket.Payload.BytesProcessed, (void *)SrcAddress,
                                          FileName);
                        /*
...
bool MM_DumpInEventCmd(const CFE_SB_Buffer_t *BufPtr)
{
    ...
    */
    ...
    if (Valid == true)
    {
        /* Run necessary checks on command parameters */
        Valid =
            MM_VerifyLoadDumpParams(SrcAddress, CmdPtr->Payload.MemType, CmdPtr->Payload.NumOfBytes, MM_VERIFY_EVENT);
